<script lang="ts">
  ...
  let demands = [22, 18, 20, 16];
  let allocation = [25, 20, 22, 18];
  let stability = 0;
  ...
  $: sectorsOk = allocation.map(
    (value, index) =>
      Number(value) >= demands[index] && Number(value) <= demands[index] + 10
  );
  ...
  function start() {
    ...
    driftTimer = setInterval(drift, 3800);
  }
  ...
  function tick() {
    ...
    if (stability >= REQUIRED_STABILITY) win();
  }
  ...
  function drift() {
    ...
    demands[index] = Math.max(12, Math.min(28, demands[index] + delta));
    const sum = demands.reduce((totalDemand, demand) => totalDemand + demand, 0);
    if (sum > 86) demands[index] -= 6;
    demands = [...demands];
  }
...
<main class="grid-page"><section class="console">
  ...
  <p class="description">{bi("Распределите не больше 100 единиц мощности. Каждый сектор должен получать не меньше запроса, но не более чем на 10 единиц больше.", "Distribute no more than 100 power units. Every sector must receive at least its demand, but no more than 10 units above it.")}</p>
