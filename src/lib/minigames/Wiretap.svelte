<script lang="ts">
  ...
  $: aligned = isWiretapLocked(controls, target, station, jammed);
  $: locked = captureArmed && aligned && !jammed;
  ...
  function tick() {
    ...
    if (locked) {
      ...
      holdProgress = Math.max(0, holdProgress - (jammed ? 0.18 : 0.28));
    }
  }
  ...
  function toggleCapture() {
    ...
    if (aligned && !jammed) captureArmed = true;
  }
...
<main class="wiretap-page">
  <section class="terminal">
    ...
    <div class="lock-box" class:locked class:jammed>
      <div>
        <span>{jammed
          ...
          : locked
            ? bi("Захват активен — корректируйте дрейф", "Capture active — correct for drift")
            : aligned
              ? bi("Цель найдена — включите захват", "Target found — arm capture")
              : captureArmed
                ? bi("Захват теряет сигнал — выровняйте оси", "Capture is losing signal — realign both axes")
    ...
    <button
      ...
      disabled={!captureArmed && (!aligned || jammed)}
      on:click={toggleCapture}
    >
      {captureArmed
        ? bi("ОТКЛЮЧИТЬ ЗАХВАТ", "DISARM CAPTURE")
        : aligned
          ? bi("ВКЛЮЧИТЬ ЗАХВАТ", "ARM CAPTURE")
          : bi("СНАЧАЛА НАЙДИТЕ СИГНАЛ", "FIND THE SIGNAL FIRST")}
    </button>
