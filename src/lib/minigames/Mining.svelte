<script lang="ts">
  ...
  const LOCKS_PER_MODULE = 4;
  const REQUIRED_LOCKS = 12;
  const GAME_TIME = 70;
  ...
  function messageText(
    ...
    return ({
      ...
      moduleReset: pick("Перегрев: текущий модуль сброшен", "Overheat: current module reset"),
      failed: pick("Время истекло. Запустите калибровку заново.", "Time expired. Start calibration again."),
  ...
  $: moduleNumber = Math.min(3, Math.floor(locks / LOCKS_PER_MODULE) + 1);
  $: moduleLocks = locks % LOCKS_PER_MODULE;
  ...
  function startGame() {
    ...
    message = "lock";
    nextReverseAt = performance.now() + 1150;
    chooseTarget();
  ...
  function animate(now: number) {
    ...
    previous = now;
    const moduleIndex = Math.min(2, Math.floor(locks / LOCKS_PER_MODULE));
    const speed = moduleIndex === 0
      ? 0.064
      : moduleIndex === 1
        ? 0.062 + Math.sin(now / 430) * 0.025
        : 0.078;

    if (moduleIndex === 2 && now >= nextReverseAt && cursor > 8 && cursor < 92) {
      direction *= -1;
      nextReverseAt = now + 900 + Math.random() * 650;
    }
  ...
  function lockHash() {
    ...
    if (cursorIsVisuallyInsideTarget()) {
      ...
      messageModule = Math.ceil(locks / LOCKS_PER_MODULE);
      message = locks % LOCKS_PER_MODULE === 0 ? "module" : "exact";
      if (locks >= REQUIRED_LOCKS) {
        winGame();
        return;
      }

      if (locks % LOCKS_PER_MODULE === 0) {
      ...
      const moduleStart = Math.floor(locks / LOCKS_PER_MODULE) * LOCKS_PER_MODULE;
      locks = Math.max(moduleStart, locks - 1);
  ...
  function chooseTarget() {
    const moduleIndex = Math.min(2, Math.floor(locks / LOCKS_PER_MODULE));
    targetWidth = [20, 16, 14][moduleIndex];
...
<main class="mining-page">
  <section class="reactor-card">
    <p class="eyebrow">{bi("Хеш-реактор", "Hash reactor")}</p>
    <h1>{bi("Калибровка хеш-реактора", "Hash reactor calibration")}</h1>
    <p class="description">{bi("Фиксируйте бегущий индикатор только внутри зелёного диапазона. Четыре точных хеша стабилизируют один модуль.", "Lock the moving indicator only inside the green range. Four exact hashes stabilize one module.")}</p>

    <div class="status-grid">
      <div><small>{bi("Модуль", "Module")}</small><strong>{moduleNumber}/3</strong></div>
      <div><small>{bi("Хеши модуля", "Module hashes")}</small><strong>{moduleLocks}/{LOCKS_PER_MODULE}</strong></div>
    ...
    {:else if transitioning}
      <div class="module-transition">{bi("Подключение следующего модуля…", "Connecting next module…")}</div>
    {:else if !finished}
